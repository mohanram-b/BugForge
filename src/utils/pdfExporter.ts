...
export async function exportInvestigationPdf(
  ...
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(8);
  doc.text('BUGSYNAPSE FORENSIC AI ENGINE • DIAGNOSTIC REPORT', marginX + 5, cursorY + 6.5);
  ...
  for (let p = 1; p <= totalPages; p++) {
    ...
    if (p > 1) {
      ...
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(7);
      doc.text(`BUGSYNAPSE FORENSIC REPORT • Incident ${investigation.id} • ${investigation.project}`, marginX, 10);
      doc.setDrawColor(226, 232, 240);
    ...
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(7);
    doc.text('Generated by BUGSYNAPSE Forensic AI Engine • Confidential Technical Document', marginX, pageHeight - 7);
    doc.text(`Page ${p} of ${totalPages}`, marginX + contentWidth, pageHeight - 7, { align: 'right' });
  }

  const pdfFilename = options?.filename || `BUGSYNAPSE-Report-${investigation.id}.pdf`;
