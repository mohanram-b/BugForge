...
export const SettingsView: React.FC<SettingsViewProps> = ({
  ...
  const handleSendGmailReset = async () => {
    ...
    try {
      ...
      await sendAccountPasswordReset(currentUser.email);
      const msg = `An official password reset link has been dispatched to ${currentUser.email}. Please check your inbox and spam folder.`;
      setPasswordSuccess(msg);
      triggerBottomToast('success', 'Reset Link Dispatched', `Email sent to ${currentUser.email}`);
    } catch (err: any) {
  ...
  const handleSendEmailVerification = async () => {
    ...
    try {
      ...
      const msg = `Email verification link has been dispatched to ${res.email}. Please open the link in your inbox to confirm verification.`;
      setVerificationSuccess(msg);
      triggerBottomToast('success', 'Verification Dispatched', `Email sent to ${res.email}`);
    } catch (err: any) {
        ...
        >
          ...
          {activeTab === 'security' && (
            <div className="space-y-6 max-w-2xl text-xs">
              ...
              {/* Change Website Account Password Card (2-STEP VERIFICATION FLOW) */}
              <div className="p-5 rounded-xl bg-[#0D1017] border border-[#1E2333] space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
